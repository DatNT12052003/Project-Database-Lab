...
//		presentColumn.setStyle("-fx-alignment: CENTER;");
		...
		presentColumn.setCellFactory(tc -> {
			CheckBoxTableCell<Attendance, Boolean> cell = new CheckBoxTableCell<>();
...
//	}
	...
	@FXML
	private void handleOk() {
	    StringBuilder attendanceInfo = new StringBuilder("Danh sách điểm danh:\n");

	    for (Study study : studyList) {
	        String studentId = study.getStudent().getStudentid();
	        String fullName = study.getStudent().getFullName();
	        Boolean isPresent = presentColumn.getCellData(study);


	        attendanceInfo.append("Mã SV: ").append(studentId)
	                      .append(", Họ và Tên: ").append(fullName)
	                      .append(", Có mặt: ").append(isPresent)
	                      .append("\n");
	    }

	    // Hiển thị thông tin điểm danh trong một hộp thoại
	    Alert alert = new Alert(Alert.AlertType.INFORMATION);
	    alert.setTitle("Thông Tin Điểm Danh");
	    alert.setHeaderText(null);
	    alert.setContentText(attendanceInfo.toString());
	    alert.showAndWait();
	}
